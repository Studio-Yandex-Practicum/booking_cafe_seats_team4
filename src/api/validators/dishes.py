...
async def check_cafe_exists(session: AsyncSession, cafe_id: List[int]) -> List[Cafe]:
    """Проверяет, что все кафе существуют."""
    query = select(Cafe).where(Cafe.id.in_(cafe_id))
    res = await session.execute(query)
    cafes = list(res.scalars().all())
    found_ids = {int(c.id) for c in cafes}
    missing = set(cafe_id) - found_ids
    if missing:
...
def check_dish_exists(dish: Optional[Dish]) -> Dish:
    ...
    if dish is None:
        raise not_found("Такого блюда не существует.")
    return dish
...
def check_dish(dish: Optional[Dish], user: User) -> Dish:
    ...
    if not dish.is_active and user.role == UserRole.USER:
