...
from src.api.validators.users import get_user_is_admin
from src.api.deps import require_manager_or_admin
...
async def get_dishes(
    ...
    if get_user_is_admin(current_user):
...
async def create_dich(
    ...
    db_dish = await dish_crud.create_dish(session, dish_in, cafes)
    return db_dish
...
async def get_dish(
    ...
    dish_res = check_dish(dish, current_user)
    return dish_res
...
async def update_dish(
    ...
    update_dish = await dish_crud.update_dish(
        session,
        dish,
        obj_in,
    )
